...
import {
  NjHistoricalMapsService,
  NjHistoricalMapType,
} from '../../services/nj-historical-maps.service';
...
export class ArcMapComponent implements OnInit {
  ...
  public protectedLayerViewSubject: Subject<__esri.FeatureLayerView> =
    new Subject<__esri.FeatureLayerView>();
  public overlayLayersSubject: BehaviorSubject<Layer[]> = new BehaviorSubject<
    Layer[]
  >([]);
  ...
  arcgisViewReadyChange(event: any) {
    ...
    const unprotectedAreasLayer: ImageryTileLayer =
      this.unprotectedAreasService.createImageryTileLayer();
    const protectedAreasLayer: FeatureLayer =
      this.protectedAreasService.createFeatureLayer();
    ...
    mapTypes.forEach((mapType) => {
      const historicalLayer: Layer | undefined =
        this.njHistoricalMapsService.createLayer(<NjHistoricalMapType>mapType);
      if (historicalLayer) {
    ...
    this.waitForLayersToLoad(
      view,
      map,
      unprotectedAreasLayer,
      protectedAreasLayer
    );
  }
  waitForLayersToLoad(
    view: View,
    map: Map,
    unprotectedAreasLayer: ImageryTileLayer,
    protectedAreasLayer: FeatureLayer
  ): void {
